...
class Test(TestCase):
    ...
    def setUp(self):
        self.server = Server()
        self.consultant = Sender(self.server)
        self.client0 = Sender(self.server)
        self.client1 = Sender(self.server)
        self.mal = Sender(self.server)
    ...
    def test_multiple_queries_give_consistent_results(self):
        self.consultant.store_to_server("First document", [self.consultant.pk, self.client0.pk],
                                        ['id12', '2017', 'jan', 'report', 'None'])
        self.consultant.store_to_server("Second document", [self.consultant.pk, self.client0.pk],
                                        ['id12', '2017', 'jan', 'report', 'None'])
        ...
        assert len(another_outputs) == 2

    def test_malicious_clinet_can_insert_data_for_others(self):
        self.mal.store_to_server("First document", [self.consultant.pk, self.client0.pk],
                                        ['id12', '2017', 'jan', 'report', 'None'])
